...
struct return_type {
    ...
    struct promise_type {
        ...
        void return_value(People p) {
            std::cout << "return v_type = people" << std::endl;
            result_set_ = true;
        }

        void return_value(Student p) {
            std::cout << "return v_type = student" << std::endl;
            result_set_ = true;
        }
    ...
    void await_suspend(std::coroutine_handle<promise_type> h) {
        ...
        std::cout << std::this_thread::get_id() << " this promise pointer:" << (void *) (&h_.promise()) << " handle:"
                  << h_.address() << std::endl;
        std::cout << std::this_thread::get_id() << " parameter promise pointer:" << (void *) (&h.promise())
                  << " handle:" << h.address() << std::endl;
        if (!async_) {
...
return_type Foo4() {
    std::cout << std::this_thread::get_id() << " " << "enter Foo4\n";
    std::cout << std::this_thread::get_id() << " " << "resume in Foo4\n";
    People people{1};
    co_return people;
}
